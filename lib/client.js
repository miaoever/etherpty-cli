...
exports.join = function(host, token) {
  ...
  clientMeta.connect(proto + "://" + host + "/pty/meta/" + token, 'etherpty-protocol');
  ...
  clientMeta.on("connect", function(connection) {
    ...
    meta.on("join", function(data) {
      ...
      clientIO.connect(proto + "://" + host + "/pty/io/client/" + data.token, 'etherpty-protocol');
...
exports.share = function(host) {
  ...
  masterMeta.connect(proto + "://" + host + "/pty/meta/" + cols + "&" + rows, 'etherpty-protocol');
  ...
  masterMeta.on("connect", function(connection) {
    ...
    meta.on("share", function(data) {
      var proto = config.web.protocol;
      var host = config.web.host;
      var port = config.web.port;
      console.log("Your shell is shared at: " + proto + "://" + host + ":"+ port + "/" + data.token);
    });

    meta.on("start", function(data) {
      var proto = config.server.protocol;
      //build the io connection
      masterIO.connect(proto + "://" + host + '/pty/io/master/' + data.token, 'etherpty-protocol');
